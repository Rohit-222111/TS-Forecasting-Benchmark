...
results_df = pd.read_csv(DATA_DIR / "metrics.csv", index_col="Model")
FORECAST_HORIZON = 18
...
def save_fan_chart(series_idx=0):
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=h_steps, y=history.tolist(), mode="lines",
                             line=dict(color="black", width=2), name="History"))
    fig.add_trace(go.Scatter(x=f_steps, y=actual.tolist(), mode="lines",
                             line=dict(color="black", width=2, dash="dot"), name="Actual"))

    for name, res in models.items():
        fill_col, line_col = colors[name]
        fig.add_trace(go.Scatter(x=f_steps, y=res["upper"][series_idx].tolist(),
                                 line=dict(width=0), showlegend=False, fillcolor=fill_col, name=name))
        fig.add_trace(go.Scatter(x=f_steps, y=res["lower"][series_idx].tolist(),
                                 line=dict(width=0), fill="tonexty",
                                 fillcolor=fill_col, showlegend=False, name=name))
        fig.add_trace(go.Scatter(x=f_steps, y=res["median"][series_idx].tolist(),
                                 line=dict(color=line_col, width=1.5), name=name))

    fig.update_layout(template="plotly_white", height=450)
    pio.write_html(fig, CHARTS_DIR / f"fan_chart_{series_idx}.html")
